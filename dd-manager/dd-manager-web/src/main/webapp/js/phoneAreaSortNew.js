...
$('select').selectMatch();try {
    var Statistics = {
        ...
        send: function() {
            ...
            $('#LgStat').remove();
            var iframe = '<iframe id="LgStat" style="display:none" src="'+url+'"></iframe>';
            $('body').append(iframe);
        }
    };
} catch (e) {}
var LoginV1 = {
    ...
    qrCodeCheckTimeMax : 40, // 二维码检测最大次数
    loginClock      : '',
    loginTimeout    : '',
    loginInvalid    : !1,
    ...
    ajaxUrls: {
        ...
        'getFastRegCode': '/userSdk/getValidateCodeNew',
    ...
    init: function(params) {
        ...
        /*新版更多登录*/
        $('#j-moreBtn').on('mouseenter', function () {
            $('#j-moreMode').find('p').fadeIn(200);
        });

        $('#j-moreMode').on('mouseleave', function () {
            $('#j-moreMode').find('p').stop().fadeOut(200);
        });
    ...
    phoneInterval: function() {
        var count = $('#sendPhoneMsg i').length > 0 ? parseInt($('#sendPhoneMsg i').text()) : 60;
        var interval = setInterval(function() {
            count--;
            if (count <= 0) {
                clearInterval(interval);
                $("#sendPhoneMsg").html('发送动态密码').removeClass('disabled');
            }
            else {
                $("#sendPhoneMsg").html("(<i>" + count + "</i>)重新发送");
            }
        }, 1000);
    },
    ...
    showFastImgCode: function() {
        var data = LoginV1.buildBaseData();
            data.force = 1 ;
            data.method = 'LoginV1.getFastRegValidateCodeCallback';
        LoginV1.hideError();
        LoginV1.jsonp(LoginV1.config.baseUrl + LoginV1.ajaxUrls.getFastRegCode, data);
    },
    ...
    getFastRegValidateCodeCallback: function (data) {
        ...
        if (data.data.imgSrc !='' && data.data.needValidateCode == true){
            ...
            if( data.data.imgSrc.indexOf('tencentCode;') == 0 ){
                LoginV1.validateShow = true;                        // 标识 是否出腾讯验证码
            ...
            if (LoginV1.validateShow == false){
                ...
                $('#fastRegImgCode').attr('src',data.data.imgSrc).removeClass('hidden');
                $("#code-input").show();
            }
        }
    },
  ...
  // 检查手机是否合法和是否已注册
    ...
    fastRegisterSendMsg : function () {
        ...
        //图片验证发送短信
        if (!LoginV1.validateShow && LoginV1.checkVerifyFastFrom()){
            LoginV1.phoneSendMsgByCode();
        }

        //滑动验证码发送短信
        if (LoginV1.validateShow && LoginV1.checkSlideFastFrom()){
            LoginV1.slidePhoneSendMsgByCode();
        }
    },
    ...
    loginCallback: function(data) {
        ...
        if (!data || code !== 0) { // 登录失败
            ...
            if (code == -11004 && data.data && data.data.imgSrc) {
                LoginV1.showCheckCode(data.data.ywGuid, data.data.imgSrc);
                LoginV1.codeKey = data.data.sessionKey;
        ...
        if (code === 0 && data.nextAction === 0) { // 登录成功
            ...
            var loginTip = function() {
                var isShowNoveTip = LoginV1.showReadNovelTip();
                if (isShowNoveTip) {
                    setTimeout(function(){
                        $('.login-button').removeClass('loading');
                        if (isShowNoveTip) {
                            $('#nove-close').on('click', function() {
                                    $('#readNovelTip').hide();$('.mask').remove();
                                    //$('.login-wrap').show();
        ...
        if (data && data.nextAction) {
            switch (data.nextAction) {
                case 2: //重新认证静密
                    ...
                    return;
                case 8: //图片验证码
                    LoginV1.showCheckCode(data.ywGuid, data.imgSrc);
                ...
                case 11 : // 滑动验证码
                    ...
                    return;
                case 12 : // 发短信
                    LoginV1.showSendMsg(data.sessionKey,data.inputUserId);
                    return;
                default:
    ...
    loginSuccess: function(data) {
        ...
        LoginV1.redirect(data.ticket, data.ywGuid);
    },
    ...
    qqLoginQidianCallback: function(data) {
        ...
        if (LoginV1.config.type == 'wap') {
            ...
            if (ticket != '') {
                ...
                url += 'ticket=' + ticket + '&ywguid=' + ywGuid;
            }
    ...
    phoneSendMsgCallback: function(data) {
        ...
        LoginV1.phoneKey = data.data.data;
        LoginV1.phoneInterval();
    },

    //快速注册 通过图片验证获取短信验证码
    phoneSendMsgByCode:function () {
        LoginV1.hideError();
        if($('#fast-send-dpw').hasClass('disabled')) return false;
        var data = LoginV1.buildBaseData();
        var mobilePhone = $.trim($('#fast-username').val());
        var areaId = $.trim($('#j-phoneModes a.ui-select-button .ui-select-text').text());

            data.phoneIsAbroad = areaId == '+86' ? 0 : 1;
            data.inputUserId = areaId+mobilePhone,  // 86　+ 手机号
            data.sessionKey = LoginV1.codeKey,
            ...
            LoginV1.ajaxLock = true;
            $('#fast-send-dpw').addClass('disabled');

        LoginV1.jsonp(LoginV1.config.baseUrl + LoginV1.ajaxUrls.phoneSendMsg, data);
    },

  //快速注册 通过滑动验证码获取短信验证码
    slidePhoneSendMsgByCode:function () {
        if ($('#j-slideChecks').length > 0) {
            $('#cap_iframes').html('');
            var url = LoginV1.validateSrc + "&clientype=2";
            ...
            if (LoginV1.config.getslidejs == 1) {
                ...
                $.getScript(url, function () {
                    $('#j-slideChecks').show();
                    //显示验证码
                    var capOption = {callback: slidePhoneSendMsgByCodeFun, type: "embed", firstvrytype: 1};
                    capInit(document.getElementById("cap_iframes"), capOption);
                    //回调函数：验证码页面关闭时回调
                });
            }
        }
        var slidePhoneSendMsgByCodeFun = function(retJson) {
            if(retJson.ret == 0) {
                ...
                ret.phoneIsAbroad = areaId == '+86' ? 0 : 1;
                ret.inputUserId = areaId+mobilePhone,    //  86+手机号
                ret.sessionkey = LoginV1.codeKey ;
                ret.phone = mobilePhone,
                ret.type = 0, // 注册
                ret.validateCode = ticket.randstr + ';'+ ticket.ticket ,
                ret.method = 'LoginV1.slidePhoneSendMsgByCodeFunCallback';

                LoginV1.ajaxLock = true;
                $('#fast-send-dpw').addClass('disabled');

                LoginV1.jsonp(LoginV1.config.baseUrl + LoginV1.ajaxUrls.phoneSendMsg, ret);
    ...
    slidePhoneSendMsgByCodeFunCallback: function (data) {
        ...
        if(data.code == -11003 || data.code == -10092){
            $('#fast-send-dpw').removeClass('disabled');
        ...
        $('#fast-send-dpw').addClass('hasClick');
        LoginV1.sessionKey = data.data.data;  //  短信回调的sessionKey
        LoginV1.commonPhoneInterval($('#fast-send-dpw'));
    },
    ...
    phoneCodeLogin: function() {
        ...
        data.auto = LoginV1.getCookie('al') || '',
            ...
            data.sessionKey = LoginV1.phoneKey,
    ...
    showCheckCode: function(ywGuid, url) {
        LoginV1.currentType = 1;
        LoginV1.sessionKey = ywGuid;
    ...
    showSendMsg: function(sessionKey,phone){
        if(phone == "undefined" || phone == null || phone == '') return false;
        if(sessionKey == "undefined" || sessionKey == null || sessionKey == '') return false;
        ...
        var callbackFun = function(retJson) {
            if(retJson.ret == 0) {
                ...
                ret.ywtoken = LoginV1.config.ywtoken,
                    ret.code = ticket.randstr;
                ret.sig  = ticket.ticket;
    ...
    redirect: function(ticket, ywGuid) {
    ...
    gotoJumpUrl: function(data, logintype) {
        ...
        jumpUrl += '?appid=' + LoginV1.config.appId + '&ticket=' + ticket + '&ywguid=' + ywGuid + '&returnurl=' + encodeURIComponent(url) + '&jumpdm=' + LoginV1.config.jumpdm +
            '&logintype=' + logintype;
